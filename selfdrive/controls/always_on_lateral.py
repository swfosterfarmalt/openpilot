...
class AlwaysOnLateral:
  ...
  def check_aol_state(self) -> bool:
    """
    Checks if we meet the panda lateral allowed requirements based on the current AOL type
    """
    if not self.enabled:
      return False
    ...
    if self.aol_type == AlwaysOnLateralType.STOCK_OP:
      # stock openpilot only allows lateral when openpilot is fully engaged
      return self.op_active

    elif self.aol_type == AlwaysOnLateralType.CRUISE_STATE:
      # When using cruise state to check if aol is enabled we check if cruise is available (on). If we do not enable on main then we check if openpilot has previously been fully engaged.
      ...
      # When using the button state to track if aol is enabled we check both if the cruise state is available (on) and we explicitly check if the button has been toggled on. If we do not enable on main then we check if openpilot has previously been fully engaged. This is for cars where the cruise state does not directly reflect the cruise main button (Currently only HKG on op long).
      if self.main_enables:
  ...
  def update(self, car_state, op_state, car_params):
    ...
    # Always allow lateral when long controls are allowed, helps maintain state with panda
